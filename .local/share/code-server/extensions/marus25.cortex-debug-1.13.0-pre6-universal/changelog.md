...
# V1.11.3
* Issue [#861](https://github.com/Marus/cortex-debug/issues/861): Potential fix
* Issue [#867](https://github.com/Marus/cortex-debug/issues/867): STLink make it so that user has to enable `-shared` if needed. Before it was automatically added to the command-line and there was no (good) way to remove it
* Issue [#882](https://github.com/Marus/cortex-debug/issues/882): Potential fix. In some cases the extension would crash while single stepping
* Issue [#896](https://github.com/Marus/cortex-debug/issues/896): Bug in handling of OS specific overrides (like `cortex-debug.gdbPath.linux`)
...
# V1.6.4
* Issue [#741](https://github.com/Marus/cortex-debug/issues/741): Could not determine gdb version from Fedora install

# V1.6.3
* Issue [#730](https://github.com/Marus/cortex-debug/issues/730) fixed where bulk breakpoint changes would cause issues and some breakpoint actions would fail. Like enable/disable/delete all breakpoints.
* Mitigation for Issue [#725](https://github.com/Marus/cortex-debug/issues/725) with extraneous/dummy symbols masking real ones
...
* Issue [#698](https://github.com/Marus/cortex-debug/issues/698): Windows only, fixed path-name issues with `loadFiles` and a couple of other places
* PR [#700](https://github.com/Marus/cortex-debug/issues/700): RTT with JLink can now be used with any valid channel (0-15) but you can only have one channel per session which is limited by JLink server. Before, only channel#0 was allowed
...
# V1.4.4
* Bugfix: [#618](https://github.com/Marus/cortex-debug/issues/618), [#619](https://github.com/Marus/cortex-debug/issues/619): Work around a gdb bug in info-variables. gdb either takes forever or crashes or runs out of memory
...
* Issues addressed: [#599](https://github.com/Marus/cortex-debug/issues/599) [#598](https://github.com/Marus/cortex-debug/issues/598), [#539](https://github.com/Marus/cortex-debug/issues/539), [#295](https://github.com/Marus/cortex-debug/issues/295), [#601](https://github.com/Marus/cortex-debug/issues/601), [#602](https://github.com/Marus/cortex-debug/issues/602), [#593](https://github.com/Marus/cortex-debug/issues/593)
...
* Bugfix: Issue [#596](https://github.com/Marus/cortex-debug/issues/596) If neither `breakAfterReset` or `runToEntryPoint` were used, the pause button failed to work. The program would run but the debug buttons didn't do anything. We don't know which version of VSCode broke this but we created a workaround that is being tested
...
* Issue [#592](https://github.com/Marus/cortex-debug/issues/592): We were not properly detecting used TCP ports causing failures in gdb-server starts. On some systems
* Issue [#585](https://github.com/Marus/cortex-debug/issues/585): RTT Terminals were not being re-cycled. Instead, new ones were being created requiring manual cleanup.
...
* Issue [#585](https://github.com/Marus/cortex-debug/issues/585), hopefully addressed.
...
# V1.2.2
* Hotfix: Issue [#585](https://github.com/Marus/cortex-debug/issues/585) -- RTT terminal windows not getting reclaimed/cleaned-up

# V1.2.1
* Hotfix: Issue [#579](https://github.com/Marus/cortex-debug/issues/579) -- debugger won't start (hangs) if a program stopped in a function with no source file. Wrong version of the disassembler was being invoked.
...
* Issue [#561](https://github.com/Marus/cortex-debug/issues/561) more graceful handling when gdb is wedged
* Issue [#538](https://github.com/Marus/cortex-debug/issues/538): Fixed bug SVD internal debug verification. Not supposed to be for production but got released and caused false errors. This in turn resulted in SVD load failure.
* Issue [#522](https://github.com/Marus/cortex-debug/issues/522): Qemu launch failed because it does not have a matching regular expression that indicated a start. It never does and code to handle that did not work. Fixed.
* Issue [#539](https://github.com/Marus/cortex-debug/issues/539): Using GDB to get some symbol information for locals and globals. Hopefully, gives better performance for large executables. Most information still comes from objdump though.
...
* Bugfix: Issue [#561](https://github.com/Marus/cortex-debug/issues/561) more graceful handling when gdb is wedged
...
# V0.4.9
* Issue [#536](https://github.com/Marus/cortex-debug/issues/536). Typo in code for os-specific paths
...
* Issues [#524](https://github.com/Marus/cortex-debug/issues/524) and [#525](https://github.com/Marus/cortex-debug/issues/525)
* Improved handling of J-Link RTOS option (file extension), added NuttX

# V0.4.7
* Fixed a regression for STLink gdbserver. It was in fact accidentally working in prior releases. The real bug is now fixed. Issue [#494](https://github.com/Marus/cortex-debug/issues/494)
* We may have **finally** found a way to exit OpenOCD without having to kill it and OpenOCD not hanging around after the session ends. This is of course dependent on OpenOCD behaving as documented. Thanks to [#482](https://github.com/Marus/cortex-debug/issues/482) and @bohdan-tymkiv for a solution
...
* Issues [#482](https://github.com/Marus/cortex-debug/issues/482) addressed for JLink. It always cleanly exits on it own. OpenOCD is still an issue where it has to be killed which once in a blue moon does not seem to work.
* Integrated PR [#480](https://github.com/Marus/cortex-debug/issues/480) -- creates .vscode director if it doesn't exist for saving register/peripheral states.
* PRs [#489](https://github.com/Marus/cortex-debug/issues/489), [#490](https://github.com/Marus/cortex-debug/issues/490), [#488](https://github.com/Marus/cortex-debug/issues/488), [#478](https://github.com/Marus/cortex-debug/issues/478) submitted by @trond-snekvik merged. They fix issues and enhances your experience with Cortex-Debug in various ways. Thank you @trond-snekvik
...
* Terminal input
  * Support for `Paste` in RTT in RTT terminals (See Issue [#463](https://github.com/Marus/cortex-debug/issues/463))
...
1. Issues Fixed
   * Issue [#232](https://github.com/Marus/cortex-debug/issues/232): SVD Enumerated types `derivedFrom` attribute now supported (used by ST quite a lot). Caused silent SVD parsing failures before and the Peripherals window said `No SVD File Loaded`.
   * Issue [#229](https://github.com/Marus/cortex-debug/issues/229): Better handling of multiple anonymous unions and structs in the same data structure. Only the first one was shown previously, defect in VSCode really, but had to find a workaround.
   * Issue [#179](https://github.com/Marus/cortex-debug/issues/179): Depending on how the compiler was used, only static variables declared in files in the current directory were being displayed. It was an issue with how `objdump` and `gdb` behaved differently. Not a perfect fix. Use Watch Window when in doubt and report any further issues and discrepancies.
